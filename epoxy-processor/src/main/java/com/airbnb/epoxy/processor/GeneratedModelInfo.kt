...
abstract class GeneratedModelInfo(protected val memoizer: Memoizer) {
    ...
    // Attributes are sorted by name to keep their ordering consistent even when attributes are added
    // in different orders during parallel processing. Additionally this keeps overloaded methods
    // together.
    val attributeInfo: MutableSet<AttributeInfo> = TreeSet()
    ...
    fun addAttributes(attributesToAdd: Collection<AttributeInfo>) {
        ...
        attributeInfo.addAll(attributesToAdd)
    }
    ...
    fun hasEmptyConstructor(): Boolean {
        return constructors.any { it.params.isEmpty() }
    }
    ...
    class ConstructorInfo internal constructor(
    ...
    fun addAttributeGroup(
        ...
        for (attribute in attributes) {
            ...
            val hasSetExplicitDefault =
                defaultAttribute != null && hasExplicitDefault(
                    defaultAttribute
                )
